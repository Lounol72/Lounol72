...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const handleNavbarVisibility = (): void => {
  ...
  const heroButtons = document.querySelectorAll<HTMLAnchorElement>('.hero-buttons .btn');

  heroButtons.forEach((button) => {
    button.addEventListener('click', function (this: HTMLAnchorElement, e: MouseEvent) {
      const targetId = this.getAttribute('href');
      if (targetId?.startsWith('#')) {
        const targetSection = document.querySelector(targetId);
        if (targetSection) {
  ...
  if (!prefersReducedMotion) {
    ...
    const animatedElements = document.querySelectorAll<HTMLElement>('section');
  ...
  // === BACK TO TOP BUTTON ===
  const createBackToTopButton = (): void => {
